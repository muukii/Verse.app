...
struct ContentView: View {
  ...
  private func jumpToTime(player: YouTubePlayer) {
    ...
    Task {
      ...
      seekTimeText = "" // Clear the field after jumping
    }
  }
  ...
  private func isCurrentSubtitle(offset: Double) -> Bool {
    ...
    // Find the transcript that matches current playback time
    // A subtitle is current if currentTime is between its offset and the next subtitle's offset
    if let currentIndex = transcripts.firstIndex(where: { $0.offset > currentTime }) {
      // currentIndex points to the next subtitle, so check the previous one
      if currentIndex > 0 {
      ...
      // If no subtitle found after current time, check if we're on the last one
  ...
  private func updateScrollPosition() {
    ...
    // Find current subtitle and scroll to it
